...
locals {
  #
  # Special Extension settings that can be extended to Keyvault details
  #
  keyvault_settings = ["commandToExecute", "KeyVaultURL", "KeyVaultResourceId"]
}

resource "azurerm_virtual_machine_extension" "lz" {
  for_each = {
    for extension in local.azure_windows_virtual_machine_extension : extension.resource_index => extension
  }
